...
Every interactive example exposes an **About** command from its menu bar and
shows `Copyright (c) 2026 C. Klukas. All rights reserved.` in the resulting
dialog. Examples whose F1 key is contextual keep that help route separate from
About; smaller examples may use the same standard Help command for both. The
shared example-only formatter in `examples/example_about.hpp` keeps the legal
line exact without assigning ckVision's copyright to applications built by
library users.
...
| TODO | persistent Kanban, custom views, full editor notes, optimistic multi-instance conflicts | [`todo_app.cpp`](../examples/todo/todo_app.cpp) | [TODO](todo-example.md) |
...
SysInfo demonstrates a second injected platform service, live values refreshed
...
The [SysInfo guide](sysinfo-example.md) covers the complete object boundary,
measurement rules, paired graphics/fallback captures, contextual help, and
report workflow.

### TODO

TODO is the complete local-application reference: a persistent Kanban Board
with task, lane, and named-Board lifecycles; a modeless `TextEditor`; direct
drag/drop; menus, status, help, themes, and standard window management; plus
one-shot optimistic rebasing and explicit conflict resolution between running
instances.

![TODO guided Board](generated/screenshots/todo-guided.svg)

The [TODO guide](todo-example.md) is both a user tour and an implementation
map. It documents every key and pointer action, the model/codec/repository/
controller/view boundary, atomic storage and backups, all generated workflow
figures and goldens, the codec fuzzer, the real two-process proof, and focused
1,000-task benchmarks.

Every screenshot on this page is generated by its matching
`tools/docgen/capture_<name>_screenshots` target. Each target builds the shipped
example on a `HeadlessTerminal`, scripts a short interaction, and renders the
HeadlessTerminal virtual display to SVG via `tools/docgen/frame_svg`. The
virtual display consumes the exact VT/Sixel bytes
...
In the gallery's Sixel profile, Presenter first writes clean panel-colored blank cells
...
1. `examples/<name>/<name>_app.{hpp,cpp}` — the object graph, taking
   ...
   the pattern). Give every interactive example a visible About command and
   build its body with `examples::about_text(...)` so the project copyright
   notice remains present and exact.
